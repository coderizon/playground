...
- **Styling foundation**: Tailwind entry point lives in `src/styles/main.css` (with `@tailwind base/components/utilities` plus a `@layer components` block). Collect-specific cards/chips/hotkeys now live in that file so the SPA no longer depends on the orphaned `collect.css`.
- **Flow coverage**:
