...
- **Alpine component layer**:
  ...
  - Global confirm dialog + notice banners provide consistent messaging. Session discard now also routes through the shared inference guard + confirm dialog so Home behaves like the rest of the journey (stop inference if needed, then show the destructive confirm).
